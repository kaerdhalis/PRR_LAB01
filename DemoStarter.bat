...
start cmd.exe /k go run master\master.go 5010 6666 %artificialDelay%
set /a port =6060
SET /A offset=!RANDOM!%%artificialOffsetMax+ artificialOffsetMin 
for /L %%x in (1, 1, %slavecount%) do (
	SET /A offset=!RANDOM!%%artificialOffsetMax + artificialOffsetMin
	set /a port+=1
	start cmd.exe /k go run slave\slave.go !port! 127.0.0.1:5010 224.0.1.1:6666 !offset! %artificialDelay%
)